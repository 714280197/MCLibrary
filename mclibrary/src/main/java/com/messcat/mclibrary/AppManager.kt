 ...
 * 添加Activity到堆栈
 */
fun addActivitys(activity: Activity) {
 ...
 * 结束指定的Activity
 */
fun finishActivitys(activitys: Activity?) {
 ...
 * 移除指定的Activity
 */
fun removeActivitys(activitys: Activity?) {
    var activity: Activity? = activitys
    if (activity != null) {
        activityStack!!.remove(activity)
        activity = null
 ...
 * @param cls
 */
fun returnToActivitys(cls: Class<*>) {
 ...
 * @return
 */
fun isOpenActivitys(cls: Class<*>): Boolean {
    if (activityStack != null) {
        var i = 0
        val size = activityStack!!.size
        while (i < size) {
 ...
 */
fun AppExits(context: Context, isBackground: Boolean?) {
    try {
        finishAllActivitys()
...
fun goToActivity(activity: Activity, cls: Class<*>, id: String) {
    var intent: Intent = Intent(activity, cls)
    intent.putExtra("id", id)
    activity.startActivity(intent)
}

fun goToActivity(activity: Activity, cls: Class<*>, bundle: Bundle?) {
    var intent: Intent = Intent(activity, cls)
