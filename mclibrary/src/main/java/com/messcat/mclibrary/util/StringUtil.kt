 ...
 */
fun isSpace(s: String?): Boolean = s == null || s.trim { it <= ' ' }.length == 0
 ...
 */
fun null2Length0(s: String?): String = s ?: ""
 ...
 * @return 首字母大写字符串
 */
fun upperFirstLetter(s: String): String? = if (isEmpty(s) || !Character.isLowerCase(s[0])) s else (s[0].toInt() - 32).toChar().toString() + s.substring(1)
 ...
 * @return 首字母小写字符串
 */
fun lowerFirstLetter(s: String): String? = if (isEmpty(s) || !Character.isUpperCase(s[0])) s else (s[0].toInt() + 32).toChar().toString() + s.substring(1)
 ...
 * 判断字符串中是否有那个字符
 */
fun isContains(content: String, contain: String): Boolean {
